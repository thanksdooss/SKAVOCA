...
| |                                                         | |
| |  🤖 [AI 튜터의 실시간 3단 구조 피드백 (DB 캐시 + 한국어 템플릿)]       | |
| |  🔍 개념 차이점 비교:                                   | |
...
+-------------------------------------------------------------+
| * 시스템 내부 동작:                                         |
|   1. DB에 없는 오답 'tar' 감지 -> DB 캐시 피드백 API 1.0초 비동기 호출|
...
| ② | REQ-F-AI-001<br>REQ-F-AI-002 | LLM 동적 비교 해설 | DB 미등록 오답 입력 시 DB 캐시 + 한국어 템플릿가 1문장 맞춤 비교 해설 생성. 생성 즉시 DB에 영속화(Cache-Aside) | 무한한 확장성 |
